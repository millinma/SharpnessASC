...
from KFACPytorch.utils.kfac_utils import (
    ComputeCovA, ComputeCovG, ComputeMatGrad)
...
class EKFACOptimizer(torch.optim.Optimizer):
    ...
    def _update_scale(self, m):
        with torch.no_grad():
            ...
            grad_mat = self.MatGradHandler(A, S, m)
            if self.batch_averaged:
                grad_mat *= S.size(0)

            # <- this consumes too much memory!
            s_l = (self.Q_g[m] @ grad_mat @ self.Q_a[m].t()) ** 2
            s_l = s_l.mean(dim=0)
            if self.steps == 0:
